...
def topo_sort_dfs(node, visited, topo_order):
    ...
    Args:
        - node: current node to be sorted
        - visited:  a list of nodes having been visited, if a node
                    currently in the `visited`, then it should not 
                    be visited again
        - topo_order:   a list of topo-ordered nodes, if all the
                        predecessors of a node are in the topo_order,
                        then the node will be added to the topo_order
                        list

                        The topo_order is in a input first order, that is
                        if the computational graph is:
                            A -> B -> C
                        then the topo order is [A, B, C]
    """
    ### BEGIN YOUR SOLUTION
    if node in visited:
        return visited, topo_order
    visited.append(node)
    for input_node in list(node.inputs):
        visited, topo_order = topo_sort_dfs(input_node, visited, topo_order)
    topo_order.append(node) # all the previous nodes are in topo_order
    return visited, topo_order
    ### END YOUR SOLUTION
