...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        Args:
            new_shape (tuple or int): new shape of the array, e.g. (2, 3, 4), (2), (-1, 2, 3), -1, 3
        ...
        if new_shape.count(-1) > 1:
            ...
            missing_dim = prod(old_shape) // prod([dim for dim in new_shape if dim != -1])
            new_shape = tuple(missing_dim if dim == -1 else dim for dim in new_shape)

        if prod(old_shape) != prod(new_shape):
            raise ValueError("Product of shape must be equal")
    ...
    def permute(self, new_axes):
        ...
        for axis in new_axes:
            new_shape.append(old_shape[axis])
            new_strides.append(old_strides[axis])
    ...
    def broadcast_to(self, new_shape):
        """
        IMPORTANT: To use broadcast_to, must reshape first!, like (3, ) -> (1, 1, 3) -> broadcast_to (4, 2, 3)
        ...
        for i in range(len(new_shape)):
            if new_shape[i] != old_shape[i]:
                assert old_shape[i] == 1
                axes_to_broad.append(i)
        old_strides = self.strides
        new_strides = list(old_strides)
        for axis in axes_to_broad:
            new_strides[axis] = 0
        new_strides = tuple(new_strides)
        return NDArray.make(
            new_shape, strides=new_strides, device=self.device, handle=self._handle, offset=self._offset
    ...
    def __getitem__(self, idxs):
        ...
        if len(idxs) < self.ndim:
            idxs += (slice(None),) * (self.ndim - len(idxs))
        ...
        shape_slice = []
        strides_slice = []
        offset_slice = self._offset
        old_strides = self.strides
        for i, idx in enumerate(idxs):
            shape_size = math.ceil((idx.stop - idx.start) / idx.step)
            shape_slice.append(shape_size)
            strides_slice.append(old_strides[i] * idx.step)
            offset_slice += idx.start * old_strides[i]
        shape_slice = tuple(shape_slice)
        strides_slice = tuple(strides_slice)
        return NDArray.make(
            shape_slice, strides=strides_slice, device=self.device, handle=self._handle, offset=offset_slice
        )
        ### END YOUR SOLUTION
    ...
    def __setitem__(self, idxs, other):
        ...
        as __getitem__()."""
        view = self.__getitem__(idxs) # view is non-compact, because it's sliced from a compact NDArray. 
        if isinstance(other, NDArray):
    ...
    def reduce_view_out(self, axis, keepdims=False):
        """ Return a view to the array set up for reduction functions and output array. 
            view is the initial array to be reduced, out is the initial array after reduced.
            e.g. self.shape = (2, 3, 4), axis = (0, 2)
            if keepdims == False, view.shape = (3, 2, 4), out.shape = (3)
            if keepdims == True, view.shape = (3, 2, 4), out.shape = (1, 3, 1)
        """
        if isinstance(axis, tuple) and not axis:
            raise ValueError("Empty axis in reduce")

        if axis is None:
            view = self.compact().reshape((1,) * (self.ndim - 1) + (prod(self.shape),)) # e.g., (2, 3, 4) -> (1, 1, 24) 
            ...
            ) # permute axis to the last, e.g. axis = (0, 2) in (0, 1, 2),  resul => (1, 0, 2)
            out = NDArray.make(
    ...
    def flip(self, axes):
        ...
        axes: tuple, e.g. (1, 2), (1, )
        """
        ### BEGIN YOUR SOLUTION
        # Author: Qingzheng Wang
        assert isinstance(axes, tuple), "axes must be tuple, cannot be single integer. "
        offset = 0
        strides = list(self.strides)  # Create a copy of strides
        for axis in axes:
        ...
        ).compact() # compact() is for creating a copy with positive strides, 
                    # since NDArray don't support negative strides in many features, like __getitem__
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        axes: tuple, list of tuples, each tuple is a pair of integers
              e.g., given shape (2, 3, 4), axes = ((1, 1), (2, 2), (3, 3)), pad shape = (2, 7, 10)
              e.g., given shape (2, ), axes = ((1, 1)) (don't support `(1, 1)`), pad shape = (4, )
              ONLY support this case of axes for padding now!
        """
        ### BEGIN YOUR SOLUTION
        # Author: Qingzheng Wang
        # assert isinstance(axes, tuple), "axes must be a tuple"
        # assert len(axes) == self.ndim, "axes must have the same length as the number of dimensions"
        # assert all(isinstance(axis, tuple) and len(axis) == 2 for axis in axes), "each element in axes must be a tuple"
        shape_pad = []
        view_pad = []

        for i, axis in enumerate(axes):
            pad_left, pad_right = axis
            shape_pad.append(pad_left + self.shape[i] + pad_right)
            view_pad.append(slice(pad_left, pad_left + self.shape[i], 1))
        array_pad = full(shape_pad, 0, dtype=self.dtype, device=self.device)
        array_pad[tuple(view_pad)] = self
